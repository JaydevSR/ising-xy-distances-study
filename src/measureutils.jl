function ising_getconfigdata!(
    ...
    for N in lattice_sizes
        ...
        for stepT in 1:length(Temps)
            ...
            autocorr_times[stepT] = τ = ising_getcorrtime!(spins, T)
            println("   |   | Done.")

            println("   |   | Making uncorrelated measurements (τ=$(τ)) ...")
            uncorrelated_spins = ising_getuncorrconfigs!(spins, T, τ, n_uncorr)
...
function ising_getcorrtime!(spins::Matrix, T, msteps=6000)
    N = size(spins)[1]
...
function ising_getuncorrconfigs!(spins::Matrix, T, τ, n_uncorr)
    N = size(spins)[1]
    twice_τ = 2*τ
...
function ising_equilibrate_system!(spins::Matrix, T, eqsteps)
    E0, M0 = ising_total_energy(spins), ising_total_magnetization(spins)
    for i in 1:eqsteps
        E0, M0 = isingmetro_step!(spins, T, E0, M0)
    end
